...
sec=${sec:-10}
echo "Se pare ca azi cautam un pret mai mic de "$price" lei pentru" $(echo $url | cut -f 5 -d "/" | awk -F "?" '{print $1}' | sed "s/%20/ /" | tr "+" " " | tr [:lower:] [:upper:] ) "in sectiunea" $(echo $url | cut -f4 -d "/" | tr "_" " " | tr [:lower:] [:upper:])". Refresh-ul rezultatelor se va face la fiecare "$sec" secunde."
while true
do
